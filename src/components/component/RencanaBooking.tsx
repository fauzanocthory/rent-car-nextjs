   ...
   return (
    <>
      <TableRow key={bookings.id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
        ...
        <TableCell>
          <Badge color={bookings.mobil.status_booking === "Sudah Dibooking" ? "success" : "warning"}>{bookings.mobil.status_booking}</Badge>
        </TableCell>
        <TableCell>
        ...
        <TableCell>
          {bookings.status_pembayaran === "Belum Dibayar" ? (<><BayarDrawer /></>):(<><Badge color="success">Pembayaran Berhasil</Badge></>)}
        </TableCell>
      </TableRow>
    </>
  );
